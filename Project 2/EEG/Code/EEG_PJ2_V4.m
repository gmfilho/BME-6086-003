...
% Define activity times for each subject
...
for s = 1:length(subject_ids)
    ...
    % LPF and HPF
    [b_high, a_high] = butter(2, 1 / (fs / 2), 'high');
    [b_low, a_low] = butter(2, 50 / (fs / 2), 'low');
    ...
    for i = 1:size(activities, 1) - 1
        ...
        idx_end = min(round(end_time * fs), size(ica_signals, 2));  

        ica_segment = ica_signals(:, idx_start:idx_end);

       
        if isempty(ica_segment)
            warning(['Segment for activity ' activity ' is empty for ' patient_id]);
            continue;
        end
...
% Function to calculate the mean frequency
